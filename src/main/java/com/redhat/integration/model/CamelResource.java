        ...
        try (Connection con = defaultDataSource.getConnection()) {
            try (Statement statement = con.createStatement()) {
                ...
                statement.execute("CREATE TABLE orders ("
                		...
                		+ "PRIMARY KEY (id)"
                		+ ")");
                statement.execute("INSERT INTO orders values(2, 'Ansible', 10, 'ansible product', true)");
                statement.execute("INSERT INTO orders values(3, 'Openshift', 10, 'Openshift product', true)");
            }
        }
    }
}